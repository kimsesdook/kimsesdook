<!-- 
💡 이 코드를 복사해서 본인의 깃허브 계정명과 동일한 이름의 리포지토리(kimsesdook/kimsesdook)의 README.md 파일에 덮어쓰기 하시면 됩니다! 
-->

<div align="center">

<!-- 타이틀 및 타이핑 효과 애니메이션 -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=26&pause=1000&color=2E86C1&center=true&vCenter=true&width=500&lines=Hi+there!+%F0%9F%91%8B;I'm+a+Backend+Developer;Building+Scalable+Architecture;Welcome+to+my+GitHub+Profile!" alt="Typing SVG" />

<br/>

<!-- 방문자 수 뱃지 (안정적인 버전) -->
<img src="https://komarev.com/ghpvc/?username=kimsesdook&color=blue&style=flat-square" alt="Profile views" />

</div>

<br/>

## 👨‍💻 About Me

**"비즈니스 로직의 깊이를 이해하고, 데이터 기반으로 성능을 최적화하는 개발자입니다."**

* 🌱 **현재 주력 분야:** Java, Spring Boot 생태계를 기반으로 한 **Domain-Driven Layered Architecture** 설계 및 API 개발
* 🛠️ **주요 관심사:** 
  * N+1 문제, 복잡한 JPA 쿼리 등 **병목 현상 원인 파악 및 성능 튜닝**
  * Redis를 활용한 데이터 캐싱 및 실시간 데이터 처리
  * 직관적이고 확장 가능한 시스템 아키텍처 구축
* 🔭 **현재 활동:** 직장인 웰빙 관리 HR-SaaS 플랫폼 **`CalmDesk`** 백엔드 고도화 및 **`ClassLink ERP`** 프로젝트 진행 중
* 📫 **연락처:** [이메일 주소 입력] 혹은 [블로그/노션 링크 입력]

<br/>

## 🚀 Pinned Projects (주요 프로젝트)

제가 시스템 설계 및 백엔드 개발에 깊게 참여한 주요 프로젝트들입니다.

| 프로젝트명 | 설명 | 기술 스택 | 링크 |
|:---:|:---|:---:|:---:|
| **CalmDesk<br>(Backend)** | **[HR-SaaS 웰빙 & 근태 관리 시스템]**<br>∙ 생체 데이터 기반 스트레스 분석 및 대시보드 제공<br>∙ 실시간 출퇴근 관리 및 보상 기반 웰빙 포인트/기프티콘 스토어 구현<br>∙ *JPA 성능 최적화(N+1 해결) 및 Redis 기반 캐싱/세션 관리 도입* | Java 17, Spring Boot, MySQL, Redis, Spring Security (JWT) | [Repo](https://github.com/kimsesdook/CalmdeskBackend) |
| **CalmDesk<br>(Frontend)** | **[HR-SaaS 프론트엔드 구축]**<br>∙ 직원 편의를 위한 웰빙 연동 출근 및 마이페이지 통합 관리 UI 개발 | JavaScript (ES6+), HTML5, CSS3 | [Repo](https://github.com/kimsesdook/CallcalmFrontend) |
| **ClassLink<br>ERP** | **[기업용 전사적 자원 관리 웹 애플리케이션]**<br>∙ 레거시 시스템 구조 이해 및 비즈니스 로직 웹 서비스화 | Java, JSP/Servlet, Oracle/MySQL | [Repo](https://github.com/kimsesdook/ClassLinkErpProject) |

<br/>

## 🛠️ Tech Stack (기술 스택)

> 꾸준히 실무에 적용하고, 깊이 파고드는 기술들입니다.

### 🍃 Backend & Frameworks
<p>
  <img src="https://img.shields.io/badge/Java 17-007396?style=for-the-badge&logo=java&logoColor=white"/> 
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white"/> 
  <img src="https://img.shields.io/badge/Spring_Security-6DB33F?style=for-the-badge&logo=Spring-Security&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_Data_JPA-6DB33F?style=for-the-badge&logo=spring&logoColor=white"/>
  <img src="https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=JSON-web-tokens&logoColor=white"/>
</p>

### 🗄️ Database & Caching
<p>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white"/>
  <img src="https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=Hibernate&logoColor=white"/>
</p>

### 🐳 Infrastructure & DevOps
<p>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white"/>
  <img src="https://img.shields.io/badge/Gradle-02303A?style=for-the-badge&logo=gradle&logoColor=white"/>
</p>

### 💻 Frontend (Basic)
<p>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
</p>

### ⚙️ Tools
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  <img src="https://img.shields.io/badge/IntelliJ_IDEA-000000?style=for-the-badge&logo=intellij-idea&logoColor=white"/>
</p>

<br/>

## 📈 GitHub Stats (나의 통계)

<div align="center">
  
  <p>
    <!-- 트로피 애니메이션 추가 -->
    <a href="https://github.com/ryo-ma/github-profile-trophy">
      <img src="https://github-profile-trophy.vercel.app/?username=kimsesdook&theme=flat&no-frame=true&no-bg=true&margin-w=15" alt="kimsesdook" />
    </a>
  </p>

  <br/>

  <p>
    <!-- 깃허브 전체 Stats & 많이 쓰는 언어 -->
    <img src="https://github-readme-stats.vercel.app/api?username=kimsesdook&show_icons=true&theme=buefy&hide_border=true&title_color=2E86C1" height="150px" alt="stats" />
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=kimsesdook&layout=compact&theme=buefy&hide_border=true&title_color=2E86C1" height="150px" alt="top langs" />
  </p>
  
  <p>
    <!-- 연속 커밋(스트릭) 통계 -->
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=kimsesdook&theme=buefy&hide_border=true&stroke=0000&title_color=2E86C1" alt="GitHub Streak" />
  </p>

</div>

---

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=100&section=footer&text=Thank%20You%20For%20Visiting!&fontSize=30&fontAlignY=50&descAlignY=61&descAlign=62" width="100%"/>
</div>
